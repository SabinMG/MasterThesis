...
{
    public class InteractableBook : InteractableObjectFBBIK
    {
        public Paper[] m_papers;
        private int m_minmumPaperToTurn = 3;

        public float maxAngle = 50.0f;
        public UnityEvent m_onInteractionComplete;
        private int m_currentPaperTurnIndex = 0;
        ...
        {
            if (m_currentPaperTurnIndex == 3)
            {
                StartFlyingRemainPapers();
                m_currentPaperTurnIndex++;
            }

            if (m_currentPaperTurnIndex < m_minmumPaperToTurn)
            {
                m_papers[m_currentPaperTurnIndex].TurnPaper();
                m_currentPaperTurnIndex++;
        ...
        private void StartFlyingRemainPapers()
        {
            for (int i = m_minmumPaperToTurn - 1; i < m_papers.Length; i++)
            {
                m_papers[i].StartFlying(m_minmumPaperToTurn - i -1);
            }
        }
